...
$records = @(
	...
	New-MemoryRecord "multi-agent-improvement" "docs/hermes-multi-agent-improvement.md" "Agent guidance" "planning" "Specialized delegation roles, addon lane briefs, source-learning references, prompt packets, launch queue, queue-only output, queue selectors, integration owner, anti-duplication, dirty-state table, and output shape for agents improving agents." @("multi-agent", "agent-improvement", "specialized-role-profiles", "addon-lane-briefs", "prompt-packets", "prompt-launch-queue", "prompt-queue-json", "prompt-queue-selectors", "queue-type", "queue-id", "nousresearch-hermes-agent", "openai-codex", "delegation-roles", "integration-owner", "dirty-state-table") $generatedAt
